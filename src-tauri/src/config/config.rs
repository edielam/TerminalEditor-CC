...
// once a .cortexcode.sh file is created for bash for example, then it will not work for zsh
// because zsh needs a different approach for execute the scripts when the prompt appears

// this means that when the users switches the shell the prevoius created .cortexcode.sh file might not be of help
// TODO
...
fn update_rc_file(shell: &str) {
  ...
  let config_scripts_file = format!("{}/.cortexcode.sh", home_dir);
  let script = format!("\nsource {}\n", config_scripts_file);
